...
# Copyright 2017 Dragonsan Studios Sp. z o.o.
...
function start_server () {
    ...
    java ${JAVA_FLAGS} \
	-Dlog4j.configurationFile=${AO_BIN}/authlog.xml \
...
function stop_server () {
    ...
    if [ -e "${AO_RUN}"/auth.pid ]; then
		kill_process "authentication server  " $(cat "${AO_RUN}"/auth.pid)
	fi
}

function status_server () {
    down=0
    status_process "authentication server  " $(cat "${AO_RUN}"/auth.pid)  
    exit ${down}
}
...
# getter.
USE_CLASS_FILES=${USE_CLASS_FILES:-$(java -cp ${AO_HOME}/dist/lib/atavism.jar -Datavism.propertyfile=${AUTH_PROPERTYFILE} -Dlog4j.configurationFile=${AO_HOME}/bin/authlog.conf -DLog4jContextSelector=org.apache.logging.log4j.core.async.AsyncLoggerContextSelector atavism.scripts.PropertyGetter atavism.use_class_files)}
...
JDBC=${JDBC:-$(java -cp $AO_JAR -Datavism.propertyfile=${AUTH_PROPERTYFILE} -Dlog4j.configurationFile=${AO_HOME}/bin/authlog.conf -DLog4jContextSelector=org.apache.logging.log4j.core.async.AsyncLoggerContextSelector atavism.scripts.PropertyGetter atavism.jdbcJarPath)}

if [ $(uname -o) == "Cygwin" ]; then
    export PATH=$(cygpath "$JAVA_HOME"/bin):.:$PATH
    AO_CLASSPATH="$RHINO;$JDBC;$INJECTED_JAR;$AO_JAR;$AGIS_JAR;$GETOPT;$JYTHON;$LOG4J;$LOG4JCORE;$DISRUPTOR;$BCEL;$MANAGED;."
else
    AO_CLASSPATH="${RHINO}:${JDBC}:${INJECTED_JAR}:${AO_JAR}:${AGIS_JAR}:${GETOPT}:${JYTHON}:${LOG4J}:${LOG4JCORE}:${DISRUPTOR}:${BCEL}:${MANAGED}:."
fi
...
JVM_FLAG="${JVM_FLAG} -Xms$atavism_auth_Xms -Xmx$atavism_auth_Xmx  $AGGRESIVE"
...
case "$1" in
    ...
    test)
        ...
        ;;
          *)
        echo "usage $0 -vC start|stop|status|restart"
